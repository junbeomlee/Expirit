...
*/
  ...
  */
  $scope.getListByDay = function(e){
    ...
    document.getElementById(clickedDay).className="programDay";
    //요일 클릭했을 경우 CSS변경
    var day;
    var week = new Array('SUN', 'MON', 'TUE', 'WED', 'THUR', 'FRI', 'SAT');
    for(day in week){
      //console.log(week[day]);
      if(week[day]==clickedDay){
        document.getElementById(week[day]+"_circle").className="nocircle";
        continue;
      }else{
        document.getElementById(week[day]).className="programNo";
      }
    }
  ...
  */
  ...
  /*프로그램 날짜 선택 컨트롤러*/
  $scope.programday_checked = function(){
      console.log("SDF");
