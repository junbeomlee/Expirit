angular.module('expirit.services').service('HistoryDao',HistoryDao)

UserDao.$inject=['$cordovaSQLite','DBConnector'];

function HistoryDao($cordovaSQLite,DBConnector){

  /*사용자의 전체 히스토리 받아옴*/
  this.getAll = function(){
    return DBConnector.query("SELECT * FROM history")
    .then(function(result){
      return DBConnector.getHistory(result);
    });


  this.add = function(exercise){
    return DBConnector.query("INSERT INTO history (EMAIL, EX_NO, EX_DATE, EX_SET,EX_WEIGHT) VALUES (?,?,?,?,?)");
  };


  this.deleteAll = function(){
    return DBConnector.query("DELETE * FROM history");
  };
